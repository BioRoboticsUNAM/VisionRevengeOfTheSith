...
PlanarHorizontalSegment::PlanarHorizontalSegment( std::vector<cv::Point3f> PointsXYZ, Plane3D plane , cv::PCA pca, std::vector<cv::Point2f> convexHull2D, std::vector<cv::Point2i> indices)
{
	this->pointsXYZ = std::vector< cv::Point3f >( pointsXYZ ); 
	this->plane = Plane3D( plane ); 
	this->pca = cv::PCA(pca); 
