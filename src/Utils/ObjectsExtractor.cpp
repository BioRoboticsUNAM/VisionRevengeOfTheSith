...
std::vector<DetectedObject> ObjectsExtractor::ExtractObjectsHorizantalPlanes( cv::Mat rgbImage, cv::Mat pointsCloud, cv::Mat& outObjectsMask, float pan, float tilt, float headZ ){

	outObjectsMask = cv::Mat::zeros(pointsCloud.rows, pointsCloud.cols, CV_8UC1); 

	// Getting Normals
	bool normalsBlur = true; 
	int normalsBlurSize = 9; 
	...
	cv::Mat planesMask; 
	cv::findNonZero( horNormalsMask , horNorMat ); 
	...
	for( size_t plane=0 ; plane<horizontalPlanes.size() ; plane++ ){
		...
		ConnectedComponents3D(pointsCloud, objectsByPlaneMask, 0.02, cv::Mat(), objIndexes, objMasks); 

		for( int i=0; i<objIndexes.size(); i++){
			if( objIndexes[i].size() < 200 )
				continue; 
